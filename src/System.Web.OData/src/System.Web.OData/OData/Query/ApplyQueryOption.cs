...
namespace System.Web.OData.Query
{
    ...
    public class ApplyQueryOption
    {
        ...
        public IQueryable ApplyTo(IQueryable query, ODataQuerySettings querySettings)
        {
            ...
            {
                if (transformation.Kind == TransformationNodeKind.Aggregate ||
                    transformation.Kind == TransformationNodeKind.GroupBy)
                {
                    var binder = new AggregationBinder(updatedSettings, _assembliesResolver, ResultClrType,
                ...
                else if (transformation.Kind == TransformationNodeKind.Filter)
                {
                    ...
                    Expression filter = FilterBinder.Bind(query, filterTransformation.FilterClause, ResultClrType,
