...
namespace Microsoft.OData.UriParser
{
    ...
    public sealed class UriQueryExpressionParser
    {
        ...
        internal AggregateExpressionToken ParseAggregateExpressionFilteredCount(QueryToken expression)
        {
            ...
            AggregationMethodDefinition verb = AggregationMethodDefinition.FilteredCount;

            // =
            if (this.lexer.CurrentToken.Kind != ExpressionTokenKind.Equal)
            {
                throw ParseError(
                    ODataErrorStrings.UriQueryExpressionParser_OpenParenExpected(this.lexer.CurrentToken.Position,
                        this.lexer.ExpressionText));
            }
            var filter = ParseParenExpression();

            var alias = this.ParseAggregateAs();

            return new AggregateExpressionToken(expression, verb, alias.Text, filter);
        }
        ...
        internal GroupByToken ParseGroupBy()
        {
            ...
            AggregateToken aggregateToken = null;
            ...
            if (this.lexer.CurrentToken.Kind == ExpressionTokenKind.Comma)
            {
                ...
                {
                    aggregateToken = this.ParseAggregate();
                    transformationToken = aggregateToken;
                }
                else
                {
            ...
            return new GroupByToken(properties, transformationToken, aggregateToken);
        }
        ...
        {
            if (this.lexer.CurrentToken.Kind != ExpressionTokenKind.OpenParen && this.lexer.CurrentToken.Kind != ExpressionTokenKind.Equal)
            {
                throw ParseError(ODataErrorStrings.UriQueryExpressionParser_OpenParenExpected(this.lexer.CurrentToken.Position, this.lexer.ExpressionText));
            }
