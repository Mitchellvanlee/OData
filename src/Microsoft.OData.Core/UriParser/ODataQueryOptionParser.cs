...
namespace Microsoft.OData.UriParser
{
    ...
    public class ODataQueryOptionParser
    {
        ...
        public ODataQueryOptionParser(IEdmModel model, IEdmType targetEdmType,
            IEdmNavigationSource targetNavigationSource, IDictionary<string, string> queryOptions)
            ...
            IServiceProvider container)
        {
            ...
            {
                ParameterAliasValueAccessor = new ParameterAliasValueAccessor(queryOptions
                    .Where(_ => _.Key.StartsWith("@", StringComparison.Ordinal)).ToDictionary(_ => _.Key, _ => _.Value))
            };
        }
        ...
        public SelectExpandClause ParseSelectAndExpand()
        {
            ...
            this.selectExpandClause =
                ParseSelectAndExpandImplementation(selectQuery, expandQuery, this.Configuration, this.odataPathInfo);
            return this.selectExpandClause;
        }
        ...
        {
            string skipQuery;
            return this.TryGetQueryOption(UriQueryConstants.SkipQueryOption, out skipQuery)
                ? ParseSkip(skipQuery)
                : null;
        }
        ...
        {
            string countQuery;
            return this.TryGetQueryOption(UriQueryConstants.CountQueryOption, out countQuery)
                ? ParseCount(countQuery)
                : null;
        }
        ...
        public string ParseSkipToken()
        {
            ...
            return this.TryGetQueryOption(UriQueryConstants.SkipTokenQueryOption, out skipTokenQuery)
                ? skipTokenQuery
                : null;
        }
        ...
        public string ParseDeltaToken()
        {
            ...
            return this.TryGetQueryOption(UriQueryConstants.DeltaTokenQueryOption, out deltaTokenQuery)
                ? deltaTokenQuery
                : null;
        }
        ...
        /// <returns>A <see cref="FilterClause"/> representing the metadata bound filter expression.</returns>
        private FilterClause ParseFilterImplementation(string filter, ODataUriParserConfiguration configuration,
            ODataPathInfo odataPathInfo)
        {
            ...
            state.ImplicitRangeVariable =
                NodeFactory.CreateImplicitRangeVariable(odataPathInfo.TargetEdmType.ToTypeReference(),
                    odataPathInfo.TargetNavigationSource);
        ...
        private static ApplyClause ParseApplyImplementation(string apply, ODataUriParserConfiguration configuration,
            ODataPathInfo odataPathInfo)
        {
            ExceptionUtils.CheckArgumentNotNull(configuration, "configuration");
        ...
        private static SelectExpandClause ParseSelectAndExpandImplementation(string select, string expand,
            ODataUriParserConfiguration configuration, ODataPathInfo odataPathInfo)
        {
            ExceptionUtils.CheckArgumentNotNull(configuration, "configuration");
            ...
            SelectExpandSyntacticParser.Parse(select, expand, odataPathInfo.TargetStructuredType, configuration,
                out expandTree, out selectTree);
        ...
        private OrderByClause ParseOrderByImplementation(string orderBy, ODataUriParserConfiguration configuration,
            ODataPathInfo odataPathInfo)
        {
            ExceptionUtils.CheckArgumentNotNull(configuration, "configuration");
            ExceptionUtils.CheckArgumentNotNull(configuration.Model, "model");
            ...
            UriQueryExpressionParser expressionParser = new UriQueryExpressionParser(
                configuration.Settings.OrderByLimit, configuration.EnableCaseInsensitiveUriFunctionIdentifier);
        ...
        private static ComputeClause ParseComputeImplementation(string compute,
            ODataUriParserConfiguration configuration, ODataPathInfo odataPathInfo)
        {
            ExceptionUtils.CheckArgumentNotNull(configuration, "configuration");
        ...
        private bool TryGetQueryOption(string name, out string value)
        {
            ...
            StringComparison stringComparison =
                isCaseInsensitiveEnabled ? StringComparison.OrdinalIgnoreCase : StringComparison.Ordinal;

            string nameWithoutDollarPrefix =
            (isNoDollarQueryOptionsEnabled &&
             trimmedName.StartsWith(UriQueryConstants.DollarSign, StringComparison.Ordinal))
                ? trimmedName.Substring(1)
                : null;

            var list = this.queryOptions
                .Where(pair => string.Equals(trimmedName, pair.Key, stringComparison)
                               || (nameWithoutDollarPrefix != null && string.Equals(nameWithoutDollarPrefix, pair.Key,
                                       stringComparison)))
                .ToList();
            ...
            throw new ODataException(Strings.QueryOptionUtils_QueryParameterMustBeSpecifiedOnce(
                isNoDollarQueryOptionsEnabled
                    ? string.Format(CultureInfo.InvariantCulture, "${0}/{0}", nameWithoutDollarPrefix ?? trimmedName)
                    : trimmedName));
        }
